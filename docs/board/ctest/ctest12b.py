...
if __name__ == '__main__':
    ...
    for i in range(vnum): #行
        for j in range(hnum): #列
            ...
            DrawingPanel.put(trans=crt.Translate(x=hspan*j, y=vspan*i), child=D)
            LEAF_LIST.append(D)
            ## セルオブジェクトC 
            oid += 1

            # #マーカー
            # N = bd.DrawCircle(r=1, source_rgb=crt.MYCOL['red'], fill='fill')
            # RowBoard.put(trans=crt.Translate(x=hspan*j, y=0), child=N)
    ...
    # # DrawingPanel.put(trans=crt.Translate(x=dskip, y=dskip), child=MarkerBoard)
    # for i in range(vnum+1): #row
    #     for j in range(hnum+1): #column
            
    #         #描画オブジェクト: Cross marker
    #         M = bd.DrawMarkerCross(ticklen=4, rgb=crt.MYCOL['blue'])
    #         MarkerBoard.put(trans=crt.Translate(x=hspan*j, y=vspan*i), child=M)

            
    #         #===
    #         # #描画オブジェクト: circle
    #         # M = bd.DrawCircle(x=0, y=0, r=1.5,
    #         #                   line_width=0.5,
    #         #                   anchor_x='center', anchor_y='mid',
    #         #                   rgb=crt.MYCOL['cyan'], fill='fill', 
